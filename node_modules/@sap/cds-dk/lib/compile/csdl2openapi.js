 ...
 *
 * Latest version: https://github.wdf.sap.corp/d037427/odata-openapi/blob/master/lib/csdl2openapi.js
*/
 ...
 * @param {object} pCsdl CSDL document
...
module.exports.csdl2openapi = function (
    ...
    {
        url: gServiceRoot,
        scheme: pScheme = 'https',
        host: pHost = 'localhost',
        basePath: pBasePath = '/service-root',
        diagram: pDiagram = false,
        maxLevels: pMaxLevels = 5
    } = {}
) {
    gServiceRoot = gServiceRoot || (pScheme + '://' + pHost + pBasePath);
    const gQueryOptionPrefix = pCsdl.$Version <= '4.0' ? '$' : '';
    const gTypesToInline = {}; // filled in schema() and used in inlineTypes()
    const gBoundOverloads = {};
    const gDerivedTypes = {};
    const gAlias = {};
    const gNamespace = { 'Edm': 'Edm' };
    const gNamespaceUrl = {};
    const gVoc = {};
    preProcess(pCsdl, gBoundOverloads, gDerivedTypes, gAlias, gNamespace, gNamespaceUrl, gVoc);

    const gEntityContainer = pCsdl.$EntityContainer ? modelElement(pCsdl.$EntityContainer) : {};
    const gKeyAsSegment = gEntityContainer[gVoc.Capabilities.KeyAsSegmentSupported];
    const gDeepUpdate = gEntityContainer[gVoc.Capabilities.DeepUpdateSupport] && gEntityContainer[gVoc.Capabilities.DeepUpdateSupport].Supported;

    const gOpenapi = {
        openapi: '3.0.2',
        info: getInfo(pCsdl, gEntityContainer),
        servers: getServers(gServiceRoot),
        tags: getTags(gEntityContainer),
        paths: getPaths(gEntityContainer),
        components: getComponents(pCsdl, gEntityContainer)
    };
    if (!pCsdl.$EntityContainer) {
        delete gOpenapi.servers;
        delete gOpenapi.tags;
    }

    security(gOpenapi, gEntityContainer);
     ...
     * @param {object} csdl CSDL document
     * @param {object} pEntityContainer Entity Container object
     * @return {object} Info Object
     */
    function getInfo(csdl, pEntityContainer) {
        ...
        const description = (
            pEntityContainer[gVoc.Core.LongDescription]
            || containerSchema[gVoc.Core.LongDescription]
            || 'This service is located at [' + gServiceRoot + '/]('
            + gServiceRoot.replace(/\(/g, '%28').replace(/\)/g, '%29') + '/)'
        ) + (pDiagram ? getResourceDiagram(csdl, pEntityContainer) : '');
        return {
            title: pEntityContainer[gVoc.Core.Description]
                || containerSchema[gVoc.Core.Description]
                || (csdl.$EntityContainer ? 'Service for namespace ' + namespace : 'OData CSDL document'),
            description: csdl.$EntityContainer ? description : '',
            version: containerSchema[gVoc.Core.SchemaVersion] || ''
        };
    }
            ...
            if (resource.$Type) {
                ...
                if (resource.$Action) {
                    ...
                    if (overload.$ReturnType) {
                        ...
                        if (type) {
                            diagram += '-'
                                + cardinality(overload.$ReturnType)
                    ...
                    if (overloads) {
                        ...
                        const iOverload = unbound[0];
                        const type = modelElement(iOverload.$ReturnType.$Type || 'Edm.String');
                        if (type) {
                            diagram += '-'
                                + cardinality(iOverload.$ReturnType)
                                + '>[' + nameParts(iOverload.$ReturnType.$Type).name + ']';
                        }
                    }
                }
            }
    ...
    function modelElement(qname) {
        ...
        const schema = pCsdl[q.qualifier] || pCsdl[gNamespace[q.qualifier]];
    ...
    function getTags(container) {
        ...
        Object.keys(container).filter(name => isIdentifier(name) && container[name].$Type).forEach(child => {
            ...
            const description = container[child][gVoc.Core.Description]
                || type && type[gVoc.Core.Description];
    ...
    function navigationPropertyRestrictions(root, navigationPath) {
        const navigationRestrictions = root[gVoc.Capabilities.NavigationRestrictions] || {};
    ...
    function nonExpandableProperties(root, navigationPath) {
        const expandRestrictions = root[gVoc.Capabilities.ExpandRestrictions] || {};
    ...
    function pathItemsWithKey(paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level, navigationPath, restrictions, nonExpandable) {
        const targetIndexable = target == null || target[gVoc.Capabilities.IndexableByKey] != false;
    ...
    function operationCreate(pathItem, element, name, sourceName, targetName, target, level, restrictions) {
        const insertRestrictions = restrictions.InsertRestrictions || target && target[gVoc.Capabilities.InsertRestrictions] || {};
        ...
        if (insertRestrictions.Insertable !== false) {
            ...
            pathItem.post = {
                ...
                tags: [sourceName],
                requestBody: {
                    description: type && type[gVoc.Core.Description] || 'New ' + lname,
                    required: true,
                    content: {
                        'application/json': {
    ...
    function operationRead(pathItem, element, name, sourceName, targetName, target, level, restrictions, byKey, nonExpandable) {
        const targetRestrictions = target && target[gVoc.Capabilities.ReadRestrictions];
        ...
        if (readable) {
            ...
            const deltaSupported = element[gVoc.Capabilities.ChangeTracking] && element[gVoc.Capabilities.ChangeTracking].Supported;
            if (!byKey && deltaSupported) {
                operation.responses[200].content['application/json'].schema.properties['@odata.deltaLink'] = {
                    type: 'string',
                    example: pBasePath + '/' + name + '?$deltatoken=opaque server-generated token for fetching the delta'
                }
            }
            ...
            if (collection) {
                ...
                if (pCsdl.$Version >= '4.0') optionSearch(operation.parameters, target, restrictions);
    ...
    function optionCount(parameters, target) {
        const targetRestrictions = target && target[gVoc.Capabilities.CountRestrictions];
    ...
    function optionExpand(parameters, element, target, nonExpandable) {
        const targetRestrictions = target && target[gVoc.Capabilities.ExpandRestrictions];
        ...
        if (supported) {
            ...
            if (expandItems.length > 1) {
                parameters.push({
                    name: gQueryOptionPrefix + 'expand',
                    description: (targetRestrictions && targetRestrictions[gVoc.Core.Description])
    ...
    function navigationPaths(element, prefix = '', level = 0) {
        ...
        Object.keys(properties).forEach(key => {
            ...
            } else if (properties[key].$Type && level < pMaxLevels) {
    ...
    function optionFilter(parameters, target, restrictions) {
        const filterRestrictions = restrictions.FilterRestrictions || target && target[gVoc.Capabilities.FilterRestrictions] || {};

        if (filterRestrictions.Filterable !== false) {
            const filter = {
                name: gQueryOptionPrefix + 'filter',
                description: filterRestrictions[gVoc.Core.Description]
    ...
    function optionOrderBy(parameters, element, target, restrictions) {
        const sortRestrictions = restrictions.SortRestrictions || target && target[gVoc.Capabilities.SortRestrictions] || {};
        ...
        if (sortRestrictions.Sortable !== false) {
            ...
            if (orderbyItems.length > 0) {
                parameters.push({
                    name: gQueryOptionPrefix + 'orderby',
                    description: sortRestrictions[gVoc.Core.Description]
    ...
    function optionSearch(parameters, target, restrictions) {
        const searchRestrictions = restrictions.SearchRestrictions || target && target[gVoc.Capabilities.SearchRestrictions] || {};

        if (searchRestrictions.Searchable !== false) {
            if (searchRestrictions[gVoc.Core.Description]) {
                parameters.push({
                    name: gQueryOptionPrefix + 'search',
                    description: searchRestrictions[gVoc.Core.Description],
                    in: 'query',
                    schema: { type: 'string' }
                });
            } else {
    ...
    function optionSelect(parameters, element, target, restrictions) {
        const selectSupport = restrictions.SelectSupport || target && target[gVoc.Capabilities.SelectSupport] || {};
        ...
        if (selectSupport.Supported !== false) {
            ...
            if (selectItems.length > 0) {
                parameters.push({
                    name: gQueryOptionPrefix + 'select',
    ...
    function optionSkip(parameters, target, restrictions) {
        const supported = restrictions.SkipSupported !== undefined
            ...
            : target == null || target[gVoc.Capabilities.SkipSupported] !== false;
    ...
    function optionTop(parameters, target, restrictions) {
        const supported = restrictions.TopSupported !== undefined
            ...
            : target == null || target[gVoc.Capabilities.TopSupported] !== false;
    ...
    function operationUpdate(pathItem, element, name, sourceName, target, level, restrictions, byKey) {
        const updateRestrictions = restrictions.UpdateRestrictions || target && target[gVoc.Capabilities.UpdateRestrictions] || {};
        ...
        if (updateRestrictions.Updatable !== false) {
            ...
            pathItem.patch = {
                ...
                tags: [sourceName],
                requestBody: {
                    description: type && type[gVoc.Core.Description] || 'New property values',
                    required: true,
                    content: {
                        'application/json': {
    ...
    function operationDelete(pathItem, element, name, sourceName, target, level, restrictions, byKey) {
        const deleteRestrictions = restrictions.DeleteRestrictions || target && target[gVoc.Capabilities.DeleteRestrictions] || {};
        ...
        if (type && level < pMaxLevels) {
            ...
            Object.keys(properties).forEach(name => {
                ...
                if (['Recursive', 'Single'].includes(enumMember(restrictions.Navigability))
                    ...
                    const target = gEntityContainer[targetSetName];
    ...
    function navigationPathMap(type, map = {}, prefix = '', level = 0) {
        ...
        Object.keys(properties).forEach(key => {
            ...
            } else if (properties[key].$Type && !properties[key].$Collection && level < pMaxLevels) {
    ...
    function entityKey(entityType, level) {
        ...
        const gProperties = propertiesOfStructuredType(entityType);
        ...
        keys.forEach((key, index) => {
            ...
            const param = {
                description: [property[gVoc.Core.Description], property[gVoc.Core.LongDescription]].filter(t => t).join('  \n')
                    || 'key: ' + parameter,
                in: 'path',
        ...
        return { segment: (gKeyAsSegment ? '' : '(') + segment + (gKeyAsSegment ? '' : ')'), parameters: params };
    }
    ...
    function pathItemsForBoundOperations(paths, prefix, prefixParameters, element, sourceName, byKey = false) {
        const overloads = gBoundOverloads[element.$Type + (!byKey && element.$Collection ? '-c' : '')] || [];
    ...
    */
    ...
    function pathItemAction(paths, prefix, prefixParameters, actionName, overload, sourceName, actionImport = {}) {
        ...
        const pathItem = {
            post: {
                summary: actionImport[gVoc.Core.Description] || overload[gVoc.Core.Description] || 'Invoke action ' + name,
        ...
        const description = actionImport[gVoc.Core.LongDescription] || overload[gVoc.Core.LongDescription];
    ...
    function pathItemFunction(paths, prefix, prefixParameters, functionName, overload, sourceName, functionImport = {}) {
        ...
        const implicitAliases = pCsdl.$Version > '4.0' || parameters.some(p => p[gVoc.Core.OptionalParameter]);

        parameters.forEach(p => {
            const param = {
                required: implicitAliases ? !p[gVoc.Core.OptionalParameter] : true
            };
            const description = [p[gVoc.Core.Description], p[gVoc.Core.LongDescription]].filter(t => t).join('  \n');
        ...
        const pathItem = {
            get: {
                summary: functionImport[gVoc.Core.Description] || overload[gVoc.Core.Description] || 'Invoke function ' + name,
                tags: [sourceName || 'Service Operations'],
                parameters: prefixParameters.concat(params),
                responses: response(200, 'Success', overload.$ReturnType)
            }
        };
        const iDescription = functionImport[gVoc.Core.LongDescription] || overload[gVoc.Core.LongDescription];
    ...
    function pathItemBatch(paths, container) {
        const batchSupport = container[gVoc.Capabilities.BatchSupport] || {};
        const supported = container[gVoc.Capabilities.BatchSupported] !== false && batchSupport.Supported !== false;
        if (supported) {
            const firstEntitySet = Object.keys(container).filter(child => isIdentifier(child) && container[child].$Collection)[0];
            paths['/$batch'] = {
                post: {
                    summary: batchSupport[gVoc.Core.Description] || 'Send a group of requests',
                    description: (batchSupport[gVoc.Core.LongDescription] || 'Group multiple requests into a single request payload, see '
            ...
            paths['/$batch'].post.responses[pCsdl.$Version < '4.0' ? 202 : 200] = {
                ...
                'application/json': {
                    schema: type.$Collection
                        ? {
                            ...
                            properties: {
                                [pCsdl.$Version > '4.0' ? '@count' : '@odata.count']: ref('count'),
                                value: s
                            }
                        }
                        : s
    ...
    function schemaForEnumerationType(schemas, qualifier, name, type) {
        ...
        const description = type[gVoc.Core.Description];
    ...
    function schemasForStructuredType(schemas, qualifier, name, type) {
        ...
        Object.keys(properties).forEach(iName => {
            ...
            if (property.$Kind == 'NavigationProperty') {
                if (property.$Collection) {
                    allProperties[`${iName}@${pCsdl.$Version === '4.0' ? 'odata.' : ''}count`] = ref('count');
                }
                creProperties[iName] = getSchema(property, '-create');
                if (gDeepUpdate)
                    updProperties[iName] = getSchema(property, '-create');
            } else {
                if (property[gVoc.Core.Permissions] == 'Read' || property[gVoc.Core.Computed]) {
                    let index = required.indexOf(iName);
                    if (index != -1) required.splice(index, 1);
                } else {
                    creProperties[iName] = getSchema(property, '-create');
                    if (!isKey[iName] && !property[gVoc.Core.Immutable])
        ...
        if (gDerivedTypes[allName]) {
            schemas[allName].anyOf = [];
            schemas[creName].anyOf = [];
            schemas[updName].anyOf = [];
            gDerivedTypes[allName].forEach(derivedType => {
    ...
    function getParameters() {
        ...
        if (pCsdl.$Version >= '4.0') param.search = {
            name: gQueryOptionPrefix + 'search',
            in: 'query',
     ...
     */
    function getSchema(element, suffix = '', forParameter = false) {
        let s = {};
        switch (element.$Type) {
            ...
            case 'Edm.Stream':
                // eslint-disable-next-line no-case-declarations
                let jsonSchema = element[gVoc.JSON.Schema];
                if (jsonSchema) {
        ...
        if (element[gVoc.Core.Example]) {
            if (s.$ref) s = { allOf: [s] };
            s.example = element[gVoc.Core.Example].Value;
        }

        if (element[gVoc.Validation.Maximum] != undefined) {
            if (s.$ref) s = { allOf: [s] };
            s.maximum = element[gVoc.Validation.Maximum];
            if (element[gVoc.Validation.Maximum + gVoc.Validation.Exclusive]) s.exclusiveMaximum = true;
        }

        if (element[gVoc.Validation.Minimum] != undefined) {
            if (s.$ref) s = { allOf: [s] };
            s.minimum = element[gVoc.Validation.Minimum];
            if (element[gVoc.Validation.Minimum + gVoc.Validation.Exclusive]) s.exclusiveMinimum = true;
        }
        ...
        if (!forParameter && element[gVoc.Core.Description]) {
            if (s.$ref) s = { allOf: [s] };
            s.description = element[gVoc.Core.Description];
        }
    ...
    function allowedValues(schema, element) {
        const values = element[gVoc.Validation.AllowedValues];
    ...
    function getPattern(schema, element) {
        const pattern = element[gVoc.Validation.Pattern];
    ...
    function ref(typename, suffix = '') {
        ...
        if (typename.indexOf('.') != -1) {
            ...
            nsp = gNamespace[parts.qualifier];
            name = nsp + '.' + parts.name;
            url = gNamespaceUrl[nsp] || '';
    ...
    function getSecuritySchemes(components, entityContainer) {
        const authorizations = entityContainer[gVoc.Authorization.Authorizations] || [];
        const schemes = {};
    ...
    function security(openapi, entityContainer) {
        const securitySchemes = entityContainer[gVoc.Authorization.SecuritySchemes] || [];
    ...
    function namespaceQualifiedName(qualifiedName) {
        ...
        return gNamespace[np.qualifier] + '.' + np.name;
    }
