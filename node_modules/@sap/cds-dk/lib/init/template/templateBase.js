...
const { IS_WIN, PROJECT_TYPE } = require('../constants');
...
module.exports = class TemplateBase {
    ...
    getCdsModulePath() {
        return path.join(__dirname, '../../../bin/cds.js');
    }
    ...
    async getEnv(force = false) {
        if (!this.env || force) {
            ...
            const logger = (this.logger.isDebug ? this.logger : nullLogger);

            let modPath = this.getCdsModulePath();
            if (IS_WIN) {
                modPath = `"${modPath}"`;
            }
