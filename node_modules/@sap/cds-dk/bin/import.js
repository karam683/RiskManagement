...
const { fs, path, isfile, mkdirp } = cds.utils
let cwd

async function _import (file, { out, dry, into,
    'no-save':no_save,
    'no-copy':no_copy,
    ...
    const fileResolved = isfile (path.resolve(cwd,file))
    if (!fileResolved)  {
...
function _update_package_json (dest, csn) {
    try {
        ...
        if (!requires[service]) {
            const model = path.relative(cwd, dest.replace(/\.csn$/,'').replace (/\\/g,'/'))
